...
from blank_rl_lab.managers import AnimationTermCfg as AnimTerm
from blank_rl_lab.managers import MotionDataTermCfg as MotionDataTerm
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        joint_pos = ObsTerm(func=mdp.joint_pos_rel, scale=1.0, noise=Unoise(n_min=-0.01, n_max=0.01))
    ...
    class CriticCfg(ObsGroup):
        # observation terms (order preserved)
        base_lin_vel = ObsTerm(func=mdp.base_lin_vel, scale=2.00, noise=Unoise(n_min=-0.1, n_max=0.1))
        base_ang_vel = ObsTerm(func=mdp.base_ang_vel, scale=0.25, noise=Unoise(n_min=-0.2, n_max=0.2))
        projected_gravity = ObsTerm(
            func=mdp.projected_gravity,
            noise=Unoise(n_min=-0.05, n_max=0.05),
...
class AnimationCfg:
    ...
    Wraps AnimTerm to follow Isaac Lab's term-in-config pattern.
    """
    animation: AnimTerm = AnimTerm(num_steps_to_use=AMP_NUM_STEPS)
...
@configclass
class Go2RobotAMPEnvCfg(ManagerBasedAmpEnvCfg):
    ...
    def __post_init__(self):
        ...
        # Plane terrain has no terrain generator, so terrain-level curriculum cannot compute.
