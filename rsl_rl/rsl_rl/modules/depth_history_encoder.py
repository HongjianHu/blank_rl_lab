...
class DepthHistoryEncoder(nn.Module):
    ...
    def forward(self, observation, depth_image, hidden_states=None, masks=None):
        ...
        if batch_mode:
            ...
            depth_image = depth_image.flatten(0, 1) # [:, T, C, H, W] -> [:, C, H, W]
        # [:, C, H, W] -> [:, C_last, H, W] -> [:, C_last * H * W] -> [:, cnn_fc_last]
        depth_encoding = self.cnn(depth_image) 
        if not batch_mode:
            observation = observation.unsqueeze(0)
            depth_encoding = depth_encoding.unsqueeze(0)
        else:
            depth_encoding = depth_encoding.view(shape0, shape1, -1)        # [:, cnn_fc_last] -> [:, T, cnn_fc_last]
        combined_input = torch.cat((observation, depth_encoding), dim=-1)   # [:, T, cnn_fc_last] -> [:, T, cnn_fc_last + obs]
        combined_encoding = self.combination_mlp(combined_input)            # [:, T, cnn_fc_last + obs] -> [:, T, mlp_out]
        if batch_mode:
            if hidden_states is None:
                raise ValueError('Hidden states not passed to memory module during policy update')
            (rnn_out, _) = self.rnn(combined_encoding, hidden_states) # [:, T, mlp_out] -> [:, T, rnn_hidden]
             # rsl_rl的特有操作。为了并行的效率，不同长度的智能体轨迹在输入时被强行填充成了长度为T的矩阵。
             # 这一步利用 masks 矩阵将填充的无效数据剔除
            rnn_out = unpad_trajectories(rnn_out, masks)
        else:
            (rnn_out, self.hidden_states) = self.rnn(combined_encoding, self.hidden_states)
        latent_output = self.latent_output_mlp(rnn_out) # [:, T, rnn_hidden] -> [:, T, latent]
