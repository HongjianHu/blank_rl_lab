...
class StudentTeacherRecurrent(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        Normal.set_default_validate_args(False)  # Disable distribution validation for performance

    def reset(self, dones: torch.Tensor | None = None, hidden_states: tuple[HiddenState, HiddenState] = (None, None)) -> None:
        """Reset the hidden states of the RNNs."""
    ...
    def evaluate(self, obs: TensorDict) -> torch.Tensor:
        ...
        with torch.no_grad():
            if self.teacher_recurrent:
                self.memory_t.eval() # rnn includes dropout, so we need to set it to eval mode during distillation
    ...
    def get_hidden_states(self) -> tuple[HiddenState, HiddenState]:
        if self.teacher_recurrent:
            ...
            return self.memory_s.hidden_state, None # type: ignore
    ...
    def update_normalization(self, obs: TensorDict) -> None:
        if self.student_obs_normalization:
            student_obs = self.get_student_obs(obs)         # type: ignore
