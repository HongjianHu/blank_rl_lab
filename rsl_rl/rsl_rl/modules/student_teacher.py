...
class StudentTeacher(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        num_student_obs = 0
        for obs_group in obs_groups["policy"]: # obs_groups["policy"] is a list of strings, each string is a key in the obs TensorDict
            assert len(obs[obs_group].shape) == 2, "The StudentTeacher module only supports 1D observations."
            num_student_obs += obs[obs_group].shape[-1] # obs stores the actual tensor data
        num_teacher_obs = 0
        for obs_group in obs_groups["teacher"]:
            assert len(obs[obs_group].shape) == 2, "The StudentTeacher module only supports 1D observations."
            num_teacher_obs += obs[obs_group].shape[-1] # obs stores the actual tensor data   
    ...
    def _update_distribution(self, obs: TensorDict) -> None:
        ...
        if self.noise_std_type == "scalar":
            std = self.std.expand_as(mean) # expand_as is used to expand the std tensor to the same shape as mean tensor
        elif self.noise_std_type == "log": # Gradient update the logarithm of standard deviation
    ...
    def act(self, obs: TensorDict) -> torch.Tensor:
        obs = self.get_student_obs(obs)         # type:ignore
    ...
    def evaluate(self, obs: TensorDict) -> torch.Tensor:
        obs = self.get_teacher_obs(obs)         # type:ignore
    ...
    def update_normalization(self, obs: TensorDict) -> None:
        if self.student_obs_normalization:
            ...
            self.student_obs_normalizer.update(student_obs) # type:ignore
