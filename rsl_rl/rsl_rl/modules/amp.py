...
class AMPDiscriminator(nn.Module):
    def __init__(self, 
        ...
        self.disc_obs_normalizer = EmpiricalNormalization(shape=self.disc_obs_dim, eps=1.0e-4, until=1e8).to(device) #type:ignore
        ...
        if self.loss_type == LossType.WGAN:
            self.disc_output_normalizer = EmpiricalNormalization(shape=1, until=1e8).to(device) #type:ignore
        else: 
    ...
    def normalize_disc_obs(self, disc_obs: torch.Tensor) -> torch.Tensor:
        ...
        normed_disc_obs = torch.clamp(normed_disc_obs, -10.0, 10.0)
...
def resolve_amp_config(alg_cfg, obs: TensorDict, obs_groups: dict, env: VecEnv):
    if "amp_cfg" in alg_cfg and alg_cfg["amp_cfg"] is not None:
        ...
        alg_cfg["amp_cfg"]["step_dt"] = env.env.unwrapped.step_dt #type:ignore
