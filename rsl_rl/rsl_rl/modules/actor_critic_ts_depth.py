...
class ActorCriticTSDepth(nn.Module):
    def __init__(self, 
        ...
        self.depth_history_encoder = DepthHistoryEncoder(depth_image_resolution, 
                                                         num_actor_obs, 
                                                         num_latent_dims, 
                                                         cnn_input_channel, 
                                                         cnn_channel_dims, 
    ...
    # RL中常用正交初始化，在深度强化学习中，正交初始化是非常关键的trick。它能保持前向传播和反向传播时特征/梯度的方差稳定，
    # 防止深度网络中出现梯度消失或梯度爆炸，让策略网络更稳定地收敛。
    @staticmethod
