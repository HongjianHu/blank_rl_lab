...
class RolloutStorageTSDepth(FlatRolloutStorage):
    ...
    def add_transitions(self, transition: Transition) -> None:
        ...
        self.privileged_observations[self.step].copy_(transition.privileged_observations) # type:ignore 
        self.depth_image_features[self.step].copy_(transition.depth_image_features) # type:ignore
        self.critic_observations[self.step].copy_(transition.critic_observations) # type:ignore
    ...
    def _save_hidden_states(self, hidden_states) -> None:
        ...
        # [T, num_layers, num_student, hidden_dim, ...] num_student 学生策略的环境数量
    ...
    def teacher_mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        student_privileged_obs = self.privileged_observations[:, 0:self.num_student] # type:ignore
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                privileged_obs_batch = self.privileged_observations[:, start:end] # type:ignore
                ...
                hid_batch = [saved_hidden_states.permute(2, 0, 1, 3)[last_was_done][first_traj:last_traj].transpose(1, 0).contiguous() for saved_hidden_states in self.saved_hidden_states] # type:ignore
    ...
    def student_mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        privileged_observations = self.privileged_observations[:, 0:self.num_student] # type:ignore
