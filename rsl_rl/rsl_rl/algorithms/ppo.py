...
from rsl_rl.modules import ActorCritic, ActorCriticCNN, ActorCriticRecurrent, Symmetry, resolve_rnd_config, resolve_symmetry_config
from rsl_rl.modules.rnd import RandomNetworkDistillation
from rsl_rl.storage import RolloutStorage
from rsl_rl.utils import compile_model, resolve_callable, resolve_obs_groups, resolve_optimizer, string_to_callable
...
class PPO:
    ...
    def compute_returns(self, obs: TensorDict) -> None:
        ...
        advantage = 0 
        for step in reversed(range(st.num_transitions_per_env)): # rollout存取的当前时刻的价值和下一状态
    ...
    def update(self) -> dict[str, float]:
        ...
        for (
            ...
            surrogate = -torch.squeeze(advantages_batch) * ratio  # type: ignore
            surrogate_clipped = -torch.squeeze(advantages_batch) * torch.clamp(  # type: ignore
