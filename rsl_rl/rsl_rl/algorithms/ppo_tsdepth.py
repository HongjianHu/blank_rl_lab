...
class PPO_TSDepth:
    ...
    def act(self, obs, privileged_obs, depth_image_features, critic_obs):
        ...
        #    self.transition.hidden_states = self.actor_critic.get_hidden_states()
           ...
           with torch.no_grad():
               # self.actor_critic ← 学生用的，actor 会被蒸馏更新，self.teacher_actor_critic ← 教师用的，Phase 1 训完后冻住
               teacher_actions = self.teacher_actor_critic.act_inference(obs, privileged_obs) # type:ignore
    ...
    def process_env_step(self, rewards, dones, infos):
        ...
        if 'time_outs' in infos:
            if self.distillation:
                self.transition.rewards += self.gamma * torch.squeeze(self.transition.values * infos['time_outs'][:self.num_student].unsqueeze(1).to(self.device), 1) # type:ignore
            else:              
                self.transition.rewards += self.gamma * torch.squeeze(self.transition.values * infos['time_outs'].unsqueeze(1).to(self.device), 1) # type:ignore
        self.storage.add_transitions(self.transition) #type:ignore
    ...
    def update(self):
        ...
        if not self.distillation:
            generator = self.storage.teacher_mini_batch_generator(self.num_mini_batches, self.num_learning_epochs)  # type:ignore
            for (obs_batch, privileged_obs_batch, critic_obs_batch, actions_batch, target_values_batch, returns_batch, old_actions_log_prob_batch, advantages_batch, old_mu_batch, old_sigma_batch, student_obs_batch, student_privileged_obs_batch, depth_features_batch, hid_states_batch, masks_batch) in generator:
                ...
                if self.desired_kl != None and self.schedule == 'adaptive':
                    with torch.inference_mode():
                        kl = torch.sum(torch.log(sigma_batch / old_sigma_batch + 1e-05) + (torch.square(old_sigma_batch) + torch.square(old_mu_batch - mu_batch)) / (2.0 * torch.square(sigma_batch)) - 0.5, axis=-1) # type:ignore
                ...
                surrogate_loss = torch.max(surrogate, surrogate_clipped).mean() # 最大化目标函数 <-> 最小化损失
            ...
            generator = self.storage.student_mini_batch_generator(self.num_mini_batches, self.num_learning_epochs) # type:ignore
