...
learn()
│
├─ 1. env.get_observations()          → obs_td: TensorDict{polic

y, privileged, depth, critic}
├─ 2. for it in range(num_iterations):
│    │
│    ├─ ROLLOUT (× num_steps_per_env):
│    │   │
│    │   ├─ _flat_from_td(obs_td)     → actor(N,45), priv(N,~), depth(N,1200), critic(N,~)
│    │   ├─ alg.act(actor, priv, depth, critic)
│    │   │    ├─ depth_history_encoder(obs, depth) / privilege_encoder(priv) → latent
│    │   │    ├─ actor(obs + latent) → actions_mean
│    │   │    ├─ sampler → actions (samples from Normal dist)
│    │   │    ├─ critic(critic_obs) → values
│    │   │    └─ 存 transition: obs, priv, depth, critic_obs, actions, values, log_prob, mu, sigma
│    │   │
│    │   ├─ env.step(actions)         → obs_td'(下一时刻), rewards(当前时刻), dones(当前时刻), extras(当前时刻)
│    │   ├─ _nan_tripwire(...)        → 检测 NaN/Inf, 有则崩溃
│    │   └─ alg.process_env_step(rewards, dones, extras)
│    │        ├─ 超时补偿: rewards += γ·V(s_T)·time_outs
│    │        ├─ storage.add_transitions(transition)  ← 写入 buffer(当前时刻的个信息,尤其注意是当前时刻的obs)
│    │        └─ actor_critic.reset(dones)            ← 清 RNN 隐状态
│    │
│    ├─ COMPUTE_RETURNS:
│    │   ├─ _flat_from_td(obs_td) → last_critic: 最后一步观测, !!!这里的obs_td已经是下一时刻的观测了
│    │   ├─ alg.compute_returns(last_critic) delta = self.rewards[step] + ... * gamma * next_values - self.values[step]
│    │   │    └─ actor_critic.evaluate(last_critic) → last_values
│    │   │    └─ storage.compute_returns(last_values, γ, λ)
│    │   │         └─ GAE: δ=r+γ·V(sₜ₊₁)·(1-done)-V(sₜ), 从后往前算 advantage
│    │
│    ├─ UPDATE:
│    │   └─ alg.update()
│    │        ├─ teacher_mini_batch_generator()  (Phase 1) 或
│    │        │  student_mini_batch_generator()  (Phase 2)
│    │        ├─ Phase 1-子循环1: PPO loss → teacher_optimizer.step()
│    │        ├─ Phase 1-子循环2: MSE(latent_s, latent_t) → student_optimizer.step()
│    │        └─ Phase 2:          MSE(actions_s, actions_t) → student_optimizer.step()
│    │
│    ├─ log, save, detach_hidden_states (每5iter)
...
from rsl_rl.algorithms.ppo_tsdepth import PPO_TSDepth
...
class TsDepthRunner(OnPolicyRunner):
    ...
    def _construct_ts_depth_algorithm(self, obs: TensorDict) -> PPO_TSDepth:
        ...
        if distillation:
            ...
            teacher_ctor = dict(actor_hidden_dims=pt.get('actor_hidden_dims', [512, 256, 128]),
                                critic_hidden_dims=pt.get('critic_hidden_dims', [512, 256, 128]), 
                                privilege_encoder_hidden_dims=pt.get('privilege_encoder_hidden_dims', [256, 128]),
                                activation=pt.get('activation', 'elu'), 
                                init_noise_std=pt.get('init_noise_std', 1.0), 
                                clip_actions=float(pt.get('clip_actions', 100.0)))
            teacher_module = ActorCriticTSDepthTeacher(num_actor_obs, self.env.num_actions, num_priv_obs, num_latent_dims, num_critic_obs, **teacher_ctor).to(self.device) # type:ignore
            load_teacher_weights_from_ts_depth_checkpoint(teacher_module, teacher_ckpt, self.device) # type: ignore
            teacher_module.eval() # type: ignore
            for p in teacher_module.parameters(): # type: ignore
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool=False) -> None:
        ...
        '''
        get_observations() -> TensorDict (dict-like, device="cuda:0"):
        ┌──────────────┬────────────────────────────┐
        │ key          │ value shape                │
        ├──────────────┼────────────────────────────┤
        │ "policy"     │ (N, 45)                     │  ← PolicyCfg 基础观测
        │ "privileged" │ (N, ~48 + 传感器维度)        │  ← PrivilegedCfg 纯特权
        │ "depth"      │ (N, 1200)                   │  ← DepthCfg 深度图扁平
        │ "critic"     │ (N, ~5×更多)                │  ← CriticCfg 特权+基础+历史
        └──────────────┴────────────────────────────┘   
        '''
        ...
        for it in range(start_it, total_it):
            ...
            with torch.inference_mode():
                for step_idx in range(self.cfg['num_steps_per_env']):
                    ...
                    if nan_tripwire_enabled:
                        _nan_tripwire(self.env.unwrapped, obs_td, rewards, dones, actions, iteration=it, step_idx=step_idx) #type:ignore
                    if distill_slice:
                        rewards = rewards[:num_student]
                        dones = dones[:num_student]
                    self.alg.process_env_step(rewards, dones, extras) # 处理每一步的transitions
            ...
            loss_dict = {'value_function': float(mean_value_loss), 
                         'surrogate': float(mean_surrogate_loss), 
                         'latent_reconstruction': float(mean_latent_loss), 
                         'action_reconstruction': float(mean_action_rec_loss)}
    ...
    def save(self, path: str, infos: dict | None=None) -> None:
        saved_dict = {'model_state_dict': self.alg.actor_critic.state_dict(),
                      ...
                      'infos': infos}
        # 蒸馏阶段 teacher_optimizer 不会被 step，状态已过期，不保存
        if not self.alg.distillation:
            saved_dict['teacher_optimizer_state_dict'] = self.alg.teacher_optimizer.state_dict()
